...
?>
<?// intro text?>
<? include_once ($_SERVER['DOCUMENT_ROOT']."/anime.php"); ?><br><br>
...
class CPriority{
    ...
    public static function SetMeta(){
        ...
        if(!strlen(self::$arMetaParams['og:image'])){
            ...
            // VK not support .svg as og:image
        ...
        foreach(self::$arMetaParams as $metaName => $metaValue){
            if(strlen($metaValue = strip_tags($metaValue))){
                ...
                if($metaName === 'og:image'){
                    $metaValue = (CMain::IsHTTPS() ? 'https://' : 'http://').$_SERVER['SERVER_NAME'].$metaValue;
    ...
    public static function Start($siteID = 's1'){
        ...
        if(CModule::IncludeModuleEx(self::MODULE_ID) == 1)
        {
            ...
            if(!$bIndexBot)
            {
                ...
                $APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/vendor/lazysizes.min.js');
    ...
    public static function getConditionClass(){
        ...
        $class .= ' mfixed_'.$arSiteThemeOptions['HEADER_MOBILE_FIXED'];
    ...
    public static function ShowHeaderMobilePhones($class = ''){
        ...
        <?if($iCountPhones == 1):?>
            ...
            ?>
            <a href="<?=$href?>">
                <?=self::showIconSvg(SITE_TEMPLATE_PATH.'/images/svg/phone.svg');?>
            </a>
        <?elseif($iCountPhones):?>
            <!-- noindex -->
            <?=self::showIconSvg(SITE_TEMPLATE_PATH.'/images/svg/phone.svg');?>
                ...
                <div class="wrap">
                    <div class="more_phone title">
						<span class="no-decript ">
							<?=Loc::getMessage('PRIORITY_T_MENU_CALLBACK')?>
                    ...
                    <?if($arTheme["CALLBACK_BUTTON"]["VALUE"] == "Y"):?>
                        <div class="callback_wrap">
                            <span class="callback-block font_upper colored" data-event="jqm" data-param-id="<?=CPriority::getFormID("aspro_priority_callback");?>" data-name="callback"><?=GetMessage("S_CALLBACK")?></span>
                        </div>
                    <?endif;?>
